...
import { EVerified, IUser } from '../api/types/user.type';
  ...
  return (
    <div className='w-full flex justify-start items-center gap-2 py-2 bg-[#F6F6F6] px-3 rounded-lg'>
      ...
      <div className='w-full flex-grow flex flex-col justify-center items-start'>
        <p className='flex items-center gap-1 leading-none font-bold'>{user.first_name}{" "}{user.last_name} {user.verified == EVerified.verified ? <MdVerified className='text-content-color' /> : null}</p>
        <p className='text-[.8rem] text-slate-400 leading-none mt-2 font-bold'>{user.email}</p>
        <p className='text-[.8rem] text-slate-400 leading-none font-bold'>{user.phone_number}</p>
      </div>
