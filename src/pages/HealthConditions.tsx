...
function HealthConditions() {
  ...
  return (
    <form className="w-full flex flex-col flex-grow justify-start items-center pb-8" onSubmit={handleSubmit}>
      ...
      <div className="w-full px-5 flex flex-col justify-start items-start select-none mb-2">
        ...
        >
          ...
          <p className="w-full pl-7 text-slate-500 text-[.9rem] font-normal leading-5">You could either be Type I or Type II, we've got you covered.</p>
        </label>
        <label
          htmlFor="LI"
          className="flex justify-start flex-wrap py-3 items-center gap-1 text-[1rem] leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-neutral-700 font-[500]"
        >
          <Checkbox id="LI" className="border-0 bg-[#F9FAFB] mr-1" onClick={handleLIToggle} />
          <span className="select-none">Lactose Intolerance</span>
          <p className="w-full pl-7 text-slate-500 text-[.9rem] font-normal leading-5">Inability to digest lactose, which may result in abdominal pain, bloating and diarrhoea after consuming milk and other dairy products. Affects 65% of the human population.</p>
        </label>
      </div>
      <div className="w-full flex flex-col justify-start items-center flex-grow">
        <ChipsBox label="Allergies" options={allergies} detail="Food groups or ingredients you're allergic to." />
        <ChipsBox label="Please enter your meal preference(s)" options={mealPreferences} detail="This is here to make sure you end up loving the recipes we suggest." />
      </div>
      <WideButton label="Get Started" color="bg-content-color" />
    </form>
  );
}
